...
**You must never edit any file other than `findings.json`. Never touch README, classified.json, or anything else.**
...
- `tech_signals` — ASCII-only, **each ≤ 20 chars**, and **vendor-neutral for cloud services**: name the capability, not who sells it. `"Serverless Workers"` not `"CF Workers"`/`"Lambda"`, `"Edge SQL"` not `"D1"`, `"Object Storage"` not `"S3"`/`"R2"`, `"Edge Platform"` not `"Cloudflare"`/`"Vercel"`. Languages, frameworks and self-hosted infra keep their real names: `"TypeScript"`, `"Next.js"`, `"k3s"`, `"Docker"` are all fine. These feed the README chips on a public profile page, and nothing downstream rewrites them.
